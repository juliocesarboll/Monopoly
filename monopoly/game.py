...
def game():
    ...
    print(f"[GAME] PLAYERS - %s" % (players_list))
    ...
    for round in range(1, max_rounds_rule()):
        ...
        for player in players_list:
            ...
            print(f"[GAME] PLAYER POSITION - %s" % (player.position))
            print(f"[GAME] LENGTH PROPERTIES - %s" % (len(properties_list)))
    ...
    ret_list = []
    for player in len(players_list):
        ret['player_behavior'] = player.behavior
        ret['rounds'] = round
